...
namespace SIIP_Transilvania.Forms
{
    public partial class FormIncasare : Form
    {
        ...
        private void BuildTabInregistrare()
        {
            ...
            grpIncasare = MakeGroup("3. Date Incasare", 8, 166, 500, 60);
            MakeLabel("Data incasare:", 12, 24, grpIncasare);
            dtpDataIncasare = MakeDtp(100, 21, 180, grpIncasare);
            MakeLabel("Suma incasata:", 12, 44, grpIncasare);
            txtSumaIncasata = MakeText(112, 41, 200, grpIncasare);
            MakeLabel("RON", 320, 44, grpIncasare);
            txtIdIncasare = MakeText(0, 0, 1, grpIncasare, ro: true); // hidden - folosit intern
            txtIdIncasare.Visible = false;
            ...
            grpGrid.Controls.Add(new Label
            {
                ...
                Location = new System.Drawing.Point(8, 408),
                AutoSize = true,
        ...
        private void BuildTabIstoric()
        {
            ...
            grdIstoric = new DataGridView
            {
                ...
                Size = new System.Drawing.Size(990, 610),
                ReadOnly = true,
                AllowUserToAddRows = false,
                SelectionMode = DataGridViewSelectionMode.FullRowSelect,
                BackgroundColor = Color.White,
        ...
        {
            if (!(cboCaserie.SelectedItem is ComboItem ci)) return;
            _ctrl.GetFormData().SetCaserieSelectata(int.Parse(ci.Cod));
            var caserii = _ctrl.GetFormData().GetListaCaserii();
            var cas = caserii.Find(c => c.IdCaserie == int.Parse(ci.Cod));
            if (cas != null)
                txtSoldCaserie.Text = cas.SoldNumerar.ToString("F2");
        }
        ...
        private void BtnAdaugare_Click(object sender, EventArgs e)
        {
            _ctrl.DocumentNou();
            SetModeAdaugare();
            // ID generat automat de BD la salvare
        }
        ...
        private void BtnSalvare_Click(object sender, EventArgs e)
        {
            ...
            if (saved)
            {
                // Preluam ID-ul real salvat in BD — retinut in Controller inainte de reset
                int idReal = _ctrl.UltimulIdSalvat;
                txtIdIncasare.Text = idReal.ToString("D4");
        ...
        private void LoadClienti()
        {
            ...
            _ctrl.GetFormData().GetListaClienti()
                 .ForEach(c => cboClient.Items.Add(new ComboItem(c.CodClient.ToString(), c.Nume)));
        ...
        private void LoadCaseriiSiConturi()
        {
            ...
            _ctrl.GetFormData().GetListaConturi()
                 .ForEach(cb => cboIBAN.Items.Add(new ComboItem(cb.IBAN, $"{cb.IBAN} — {cb.Banca}")));
        ...
        private void RefreshGrid()
        {
            ...
            dt.Columns.AddRange(new[] {
                new DataColumn("ID"), new DataColumn("Data"),
                new DataColumn("Suma (RON)"), new DataColumn("Canal"), new DataColumn("Factura")
            });
            foreach (var i in incasari)
                dt.Rows.Add(i.IdIncasare, i.DataIncasare.ToString("dd/MM/yyyy"),
                    i.SumaIncasata.ToString("F2"), i.Canal, $"{i.SerieFact}-{i.NumarFact}");
        ...
        { var l = new Label { Text = text, Location = new System.Drawing.Point(x, y), AutoSize = true, Font = new Font("Arial", 8.5f) }; parent?.Controls.Add(l); return l; }
        private TextBox MakeText(int x, int y, int w, Control parent = null, bool ro = false, string val = "")
        { var tb = new TextBox { Location = new System.Drawing.Point(x, y), Size = new System.Drawing.Size(w, 21), ReadOnly = ro, BackColor = ro ? READONLY_COLOR : WHITE, Font = new Font("Arial", 8.5f), Text = val }; parent?.Controls.Add(tb); return tb; }
        private ComboBox MakeCombo(int x, int y, int w, Control parent = null)
        { var cb = new ComboBox { Location = new System.Drawing.Point(x, y), Size = new System.Drawing.Size(w, 21), DropDownStyle = ComboBoxStyle.DropDownList, Font = new Font("Arial", 8.5f) }; parent?.Controls.Add(cb); return cb; }
        private DateTimePicker MakeDtp(int x, int y, int w, Control parent = null)
        { var dtp = new DateTimePicker { Location = new System.Drawing.Point(x, y), Size = new System.Drawing.Size(w, 21), Format = DateTimePickerFormat.Short, Font = new Font("Arial", 8.5f) }; parent?.Controls.Add(dtp); return dtp; }
        private RadioButton MakeRadio(string text, int x, int y, bool chk, Control parent = null)
        { var rb = new RadioButton { Text = text, Location = new System.Drawing.Point(x, y), AutoSize = true, Checked = chk, Font = new Font("Arial", 8.5f) }; parent?.Controls.Add(rb); return rb; }
        private Button MakeButton(string text, int x, int y, int w, int h, Control parent, Color bg)
        { var btn = new Button { Text = text, Location = new System.Drawing.Point(x, y), Size = new System.Drawing.Size(w, h), BackColor = bg, ForeColor = Color.White, FlatStyle = FlatStyle.Flat, Font = new Font("Arial", 8.5f, FontStyle.Bold), Cursor = Cursors.Hand }; btn.FlatAppearance.BorderSize = 0; parent?.Controls.Add(btn); return btn; }
