...
namespace SIIP_Transilvania.Forms
{
    public partial class FormRetur : Form
    {
        ...
        private bool _loading = false; // previne declansarea evenimentelor in timpul setup-ului
        ...
        private void BuildTabInregistrare()
        {
            ...
            // GRP 2 — Rest disponibil pe rand separat ca sa fie vizibil complet
            grpFactInit = MakeGroup("2. Factura Initiala", 8, 76, 460, 90);
            ...
            txtDataFactInit = MakeText(84, 47, 110, grpFactInit, ro: true);
            MakeLabel("Val. totala:", 202, 50, grpFactInit);
            txtValTotala = MakeText(272, 47, 100, grpFactInit, ro: true);
            MakeLabel("Rest disponibil:", 8, 72, grpFactInit);
            txtRestDisponibil = MakeText(100, 69, 120, grpFactInit, ro: true);
            tab.Controls.Add(grpFactInit);

            // GRP 3
            grpFactRetur = MakeGroup("3. Date FacturaRetur", 8, 172, 460, 94);
            ...
            // GRP 4
            grpCanal = MakeGroup("4. Canal Plata / Incasare", 8, 272, 460, 60);
            ...
            // GRP 5
            grpDoc = MakeGroup("5. Document Generat", 8, 338, 460, 82);
            ...
            grpGrid.Controls.Add(new Label
            {
                ...
                Location = new System.Drawing.Point(8, 322),
                AutoSize = true,
        ...
        {
            if (_loading) return; // ignora evenimentul daca suntem in setup
        ...
        private void Canal_Changed(object sender, EventArgs e)
        {
            ...
            if (!num && cboIBAN.Items.Count == 0)
            {
                var ibanuri = _ctrl.GetIBANuriFirma();
                ibanuri.ForEach(iban => cboIBAN.Items.Add(iban));
        ...
        private void BtnSalvare_Click(object sender, EventArgs e)
        {
            ...
            bool saved = _ctrl.SalveazaRetur(cboMotiv.Text, cboStare.Text, val, canal, dtpDataRetur.Value.Date);
        ...
        {
            _loading = true; // blocheaza evenimentele in timp ce reconfiguram
            _modAdaugare = false;
        ...
        private void SetModeAdaugare()
        {
            ...
            _loading = false; // reactiveza evenimentele
            cboPartener.Focus();
        }
